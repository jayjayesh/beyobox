...
import 'package:beyobox_api/client.dart';
import 'package:http/http.dart' as http;

/// {@template beyobox_api_malformed_response}
/// An exception thrown when there is a problem decoded the response body.
/// {@endtemplate}
class BeyoboxApiMalformedResponse implements Exception {
  /// {@macro beyobox_api_malformed_response}
  const BeyoboxApiMalformedResponse({required this.error});

  /// The associated error.
  final Object error;
}

/// {@template beyobox_api_request_failure}
/// An exception thrown when an http request failure occurs.
/// {@endtemplate}
class BeyoboxApiRequestFailure implements Exception {
  /// {@macro beyobox_api_request_failure}
  const BeyoboxApiRequestFailure({
...
/// {@template beyobox_api_client}
/// A Dart API client for the beyobox API.
/// {@endtemplate}
class BeyoboxApiClient {
  /// Create an instance of [BeyoboxApiClient] that integrates
  /// with the remote API.
  ///
  /// {@macro beyobox_api_client}
  BeyoboxApiClient({
  ...
  Future<ArticleResponse> getArticle({
    ...
    if (response.statusCode != HttpStatus.ok) {
      throw BeyoboxApiRequestFailure(
        body: body,
  ...
  Map<String, dynamic> json() {
    try {
      ...
      Error.throwWithStackTrace(
        BeyoboxApiMalformedResponse(error: error),
        stackTrace,
      );
    }
  }
